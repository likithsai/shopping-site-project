...
const Header = (props) => {
  ...
  return (
    <header className="sticky top-0 bg-white border-gray-200 px-1.5 lg:px-6 py-2.5 border-b w-full z-10">
      <nav className="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
        ...
        <div className={`${mobileMenuOpen ? "block" : "hidden"} justify-between items-center w-full lg:flex lg:w-auto lg:order-1 transition`} id="mobile-menu-2">
          <ul className="flex flex-col my-3 lg:my-0 font-medium lg:flex-row lg:space-x-8 lg:mt-0">
            <li>
              <NavLink
                className="flex items-center py-2 pr-4 pl-3 text-gray-700 hover-bg-gray-50 lg:hover-bg-transparent lg:hover-text-primary-700 lg-p-0 aria-[current=page]:font-bold aria-[current=page]:text-blue-700"
                to="/home"
                onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
              >
